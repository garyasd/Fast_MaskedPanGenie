...
class mer_overlap_sequence_parser : public jellyfish::cooperative_pool2<mer_overlap_sequence_parser<StreamIterator>, sequence_ptr> {
  ...
  bool                     used_masked; // check if use masked
  char*                    spaced_seed; // store sapced_seed;   
  int                      spaced_weight;
  int                      spaced_len;
...
public:
  ...
  //char fsmp[51]; // store sequence left info (used for fasta)
  char fsmp[100]; // store sequence left info (used for fasta)
  mer_overlap_sequence_parser(uint16_t mer_len, uint32_t max_producers, uint32_t size, size_t buf_size,
                              StreamIterator& streams, bool masked = false, char* seed = NULL) :
    ...
    spaced_seed(seed)
  {
    ...
    if (!spaced_seed){
	 ...
	 spaced_len = std::strlen(spaced_seed);
    }
    int count = 0;
    for (int i = 0; i < spaced_len; i++){
      if (spaced_seed[i] == '1'){
        count++;
      }
    }
...
protected:
  bool open_next_file(stream_status& st) {
    ...
    return true;
     ...
     void read_fasta_masked(stream_status& st, sequence_ptr& buff) {
    auto& stream = *st.stream.standard;
    size_t read = 0;
    //const char* masked = "111111101010011100010110010011010001110010101111111"; // spaced seed
    //int spaced_len = std::strlen(spaced_seed);
    char* seq = new(char[200]); // sequence buffer
    if(st.have_seam) {
      memcpy(seq, fsmp, spaced_len - 1);
      read = spaced_len - 1;
    }
    ...
    // }
    if (stream.good()){
      read += read_sequence_masked(stream, read, seq, '>');
      if (stream.peek() == '>') {
        ignore_line(stream); // skip to next sequence
      }
    }
    //buff.end = buff.start + read;
    st.have_seam = read + 1 == 200;
    //st.have_seam = read >= (size_t)(mer_len_ - 1);
    if(st.have_seam){
      memcpy(fsmp, seq + read - spaced_len + 1, spaced_len - 1);
    }
    // store sequence into real buffer
    size_t buff_read = 0;
    for (int i = 0; i < (int)read - spaced_len + 1; i++){
      int k = 0;
      char tmp[spaced_weight + 1]; // 51mer -> 31 mer + 'N'
      for (int j = 0; j < spaced_len; j++){
        if (spaced_seed[j] == '1'){
          if (seq[j + i] == 'N') break;
          tmp[k++] = seq[j + i];
        }
      ...
      if (k == spaced_weight){
        tmp[spaced_weight] = 'N';
        memcpy(buff.start + buff_read, tmp, spaced_weight + 1);
        buff_read += spaced_weight + 1;
        ++ reads_read_;
      }
    }
    buff.end = buff.start + buff_read;
    delete[] seq;
  ...
  inline size_t read_sequence_masked(std::istream& is, const size_t read, char* const start, const char stop) {
    char le;
    return read_sequence_masked(is, read, start, stop, le);
  }

  size_t read_sequence_masked(std::istream& is, const size_t read, char* const start, const char stop, char& le) {
    size_t nread = read;
    le = '\n';
    skip_newlines(is); // Skip new lines -> get below doesn't like them
    while(is && nread < 199 && is.peek() != stop) {
      is.get(start + nread, 200 - nread);
      nread += is.gcount();
